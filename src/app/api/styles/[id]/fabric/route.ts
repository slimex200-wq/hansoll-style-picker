...
const ALLOWED_KEYS = new Set<string>([
  "sourceSheet", "pattern", "option", "fabricCode", "supplier",
  "construction", "content", "widthInch", "weightGm2", "priceYd",
  "priceLb", "finish", "yarnDetail", "comment", "fabricCountry",
  "originalText",
]);
...
export async function PATCH(
  request: NextRequest,
  ctx: RouteContext<"/api/styles/[id]/fabric">
) {
  try {
    ...
    const { data, error } = await supabase
      .from("styles")
      .update({ fabric_override: cleaned })
      .eq("id", styleId)
      .select("id, fabric_override")
      .single();

    if (error) {
      return NextResponse.json(
        { error: `Failed to update fabric override: ${error.message}` },
        { status: 500 }
      );
    }
    if (!data) {
    ...
    return NextResponse.json(
      { error: `Fabric override update failed: ${(e as Error).message}` },
      { status: 500 }
    );
  }
}
